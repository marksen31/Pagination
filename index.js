...
req.onload = function () {
  ...
  table.className = "table table-success table-striped table-hover text-sm-center";
  ...
  function createPagination(total_no_of_pages) {
    ...
    first.className = "btn btn-success btn-lg p-2 bd-highlight";
    first.innerText = 'First';
    page_container.append(first);

    var prev = document.createElement("button");
    prev.className = "btn btn-danger btn-lg p-2 bd-highlight";
    prev.innerText = 'Previous';
    page_container.append(prev);

    for (let p = 0; p < total_no_of_pages; p++) {
      var page = document.createElement("button");
      page.className = "btn btn-warning btn-lg p-2 bd-highlight";
      var id = "page" + p;
      page.setAttribute("id", id);
      page.innerText = p + 1;
      page_container.append(page);
    }
    printTable(); //initially when page is loaded and any button isn't clicked
